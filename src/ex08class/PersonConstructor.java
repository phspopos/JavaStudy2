...
public class PersonConstructor {
	...
	/*
	 디폴트 생성자 : 해당 크래스에 생성자를 정의하지 않으면 아래와 같이
	 매개변수, 실행부가 없는 생성자가 컴파일러에 의해 자동으로 삽입
	 된다. 지금까지 new 클래스명() 과 같이 사요했던 문장이 바로
	...
	public PersonConstructor() {
		/*
		 매개변수가 없어 값을 전달받을수 없으므로 항상 고정된
		 값으로만 초기화된다. 
		 */
		name = "이름없음";
		age = 1;
		addr = "미상";
		System.out.println("나는 기본생서자 입니다.");
	}
	...
	public PersonConstructor( String name ) {
		...
		age = 1;
		addr = "출처불명";
		System.out.println("나는 인자생서자[1] 입니다.");
	}
	
	//생성자3
	public PersonConstructor( String name, int age ) {
		/*
		 this()는 생성자에서 다른 생성자를 호출할때 상용한다.
		 단, 생성자 내에서만 사용할수 있고, 일반 멤버메서드에서는
		  사용할 수 없다. 
		 */
