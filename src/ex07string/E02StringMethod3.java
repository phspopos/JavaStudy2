...
public class E02StringMethod3 {
	...
	public static void main(String[] args) {
		...
		/*
		 13] split() : 문자열을 구분자를 통해 분리한 후 String 타입의
		 				배열로 반환한다. 해당 구분자가 없는 경우에는 크기가 1인 
		 				배열로 반환한다.
		 */
		System.out.println("### 13 ###");
		String phoneNumber = "010-1234-5678";
		//전화번호를 - (하이폰)으로 분리하여 크기가 3인 배열로 반환
		String[] phoneArr = phoneNumber.split("-");
		for( int i = 0; i < phoneArr.length; i++ ) {
			System.out.printf("phoneArr[%d] = %s \n" , i, phoneArr[i]);
		}
		/*
		 구분자로 사용한 문자열이 포함되어 있지 않은 경우에는 문자열
		 전체가 통째로 반환되어 배열의 크기는 1이 된다. 
		 */
		...
		/*
		 14] substring : 시작인텍스와 마지막인덱스 사이의 문자열을 
		 잘라서 반환한다.
		 	방법1 : 인데스를 하나만 사용 => 인덱스부터 문자열의 끝까지 잘라낸다.
