...
/*
 StringBuffer클래스
 : String클래스는 기본 문자열에 새로운 문자열을 추가하면 새롭게 생성되
 메모리에 문자열의 저장한다. 기존 메모리가 소멸되고 새로운 메모리가
 생성되는 낭비를 막기위해, 문자열의 변경이 많은 경우 StringBuffer를 
 사용하는 것이 좋다. 이 클래스는 기존 메모리에 문자열을 추가 (혹은 변경)
 한느 방식으로 동작한다.
 */
public class E03StringBuilderBuffer {
	...
	public static void main(String[] args) {
		...
		*/
		...
		/*
		 StrinBuffer는 문자열 저장을 위해 기본 메모리를 16으로 할당
		...
		System.out.println( "buf = " + buf );
		System.out.println( "저장된 문자열크기 : " + buf.length() );
		System.out.println( "기본버퍼의크기 : " + buf.capacity() );
